...
// Precisa viver FORA de `IntegracoesPage`: definido como componente aninhado,
// toda re-render do pai (inclusive o tique de 1s do relógio da contagem
// regressiva) criava uma nova identidade de componente a cada segundo — o
// React desmontava e remontava o card, fechando sozinho o ConfirmDialog de
// "Desconectar" antes de dar tempo de confirmar.
function ConexaoCard({
  conexao,
  aoDesconectar,
}: {
  conexao: ConexaoGoogle
  aoDesconectar: (conexao: ConexaoGoogle) => void
}) {
        ...
        <div className="flex shrink-0 items-center justify-between gap-2 sm:justify-end">
          ...
          {conexao.conectado ? (
            <ConfirmDialog
              ...
              destrutivo
              onConfirmar={() => aoDesconectar(conexao)}
              trigger={
      ...
      <div className="space-y-3">
        ...
        {isError ? (
          ...
          <div className="grid gap-3">
            {(data ?? []).map((conexao) => (
              <ConexaoCard
                key={String(conexao.dentista)}
                conexao={conexao}
                aoDesconectar={desconectarAlvo}
              />
            ))}
          </div>
        )}
